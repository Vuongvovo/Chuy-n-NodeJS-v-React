...
export default function CheckCompany() {
    ...
    const [state, setState] = useState({ page: localStorage.getItem("pageCheckCompany") || 1 })
    const { page } = state
    const actionResult = async (page) => { await dispatch(checkCompanyData(page)) }
    useEffect(() => {
        ...
        <div id="admin">
            ...
            <div className="content">
                {loading ? <div className="spin"><Spin className="mt-5" /></div> :
                    <div>
                        <Table columns={columns} pagination={false} dataSource={checkCompanys.rows.map((ok, index) => (
                            {
                                key: index + 1,
                                name: ok.name,
                                email: ok.email,
                                status: <div className="action">{ok.status === 1 ? <Link onClick={() => { handleStatus(ok.status, ok.id) }}><i className="far fa-thumbs-up "></i></Link> : <Link onClick={() => handleStatus(ok.status, ok.id)}><i className="fas fa-check"></i></Link>}</div>,
                                action:
