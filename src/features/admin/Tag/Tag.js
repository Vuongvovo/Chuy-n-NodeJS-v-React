...
export default function Tag() {
    ...
    const [state, setState] = useState({ page: localStorage.getItem("pageTag") || 1 })
    const { page } = state
    ...
    return (
        <div id="admin">
            ...
            <div className="content">
                <div className="add">
                    <Link to={`${match.url}/addTag`}><Button variant="outlined" color="secondary"><i className="fas fa-plus"></i>&nbsp;&nbsp; Thêm mới</Button></Link>
                </div>
                {loading ? <div className="spin"><Spin className="mt-5" /></div> :
                    <div>
                        <Table columns={columns} pagination={false} dataSource={tag.rows.map((ok, index) => (
                            {
                                key: index + 1,
                                name: ok.name,
