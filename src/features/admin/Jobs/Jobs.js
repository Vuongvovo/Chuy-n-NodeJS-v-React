...
import { removework, workData, updatework } from '../Slice/workSlice';
export default function Work() {
    ...
    const [state, setState] = useState({ page: localStorage.getItem("pagework") || 1 })
    const { page } = state
        ...
        <div id="admin">
            ...
            <div className="content">
                {loading ? <div className="spin"><Spin className="mt-5" /></div> :
                    <div>
                        <Table columns={columns} pagination={false} dataSource={work.rows.map((ok, index) => (
                            {
                                key: index + 1,
