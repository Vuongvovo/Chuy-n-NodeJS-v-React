    ...
    const onhandleSubmit = async (data) => {
        ...
        if (id) {
            if (img !== "") {
                ...
                const anh = await storage.ref("imagesNew").child(img.name).getDownloadURL();
                if (tag1 === tagId) {
                    dispatch(updatenew({ name: data.name, samary: data.samary, avatar: anh, content: content, id: id }));
                } else {
                    ...
                    await dispatch(updatenew({ name: data.name, samary: data.samary, avatar: anh, content: content, id: id }));
                }
            } else {
                if (checkArrayEquar(tagId, tag1)) {
                    dispatch(updatenew({ name: data.name, samary: data.samary, content: content, id: id }));
                } else {
                    await tagNewApi.deletetagNew(id);
                    var data = [];
                    for (let i = 0; i < tagId.length; i++) {
                    ...
                    console.log(data);
                    await tagNewApi.posttagNew(data);
                    await dispatch(updatenew({ name: data.name, samary: data.samary, content: content, id: id }));
                }
            }
        } else {
            await storage.ref(`imagesNew/${img.name}`).put(img);
            ...
            dispatch(addnew({ name: data.name, samary: data.samary, content: content, avatar: anh, useId: 1, status: 0, tagnew: tagnew }));
        }
        setTimeout(() => {
                ...
                <form onSubmit={handleSubmit(onhandleSubmit)}>
                    <div className="form-group">
                        ...
                        <div className="mt-3">
                            ...
                            <textarea {...register("samary", { required: true })} className="form-control w-50" id="" cols="30" rows="4"></textarea>
                    ...
                    <div className="text-center mtb">{id ? <input type="submit" value="Sửa" /> : <input type="submit" value="Thêm mới" />}{state.load ? <div className="spin"><Spin className="mt-5" /></div> : ""}</div>
                </form>
            </div>
        </div>
    )
