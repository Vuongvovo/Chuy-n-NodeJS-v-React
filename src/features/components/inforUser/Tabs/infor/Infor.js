...
export default function Infor({ id }) {
    const [state, setState] = useState({ typeofworkId: "", typeofworkCheck: '', tagId: "", tagCheck: "", loading: false, linkImg: "", tenanh: "", img: "", anh: "", linkImgBanner: "", tenanhBanner: "", imgBanner: "", anhBanner: "" })
    const { loading, linkImg, tenanh, img, typeofworkId, anh, linkImgBanner, tenanhBanner, typeofworkCheck, tagCheck, imgBanner, anhBanner, tagId } = state;
    ...
    const actionResult = async (page) => { await dispatch(userData(page)) }
    const edit = async (data) => {
        ...
        if (data.anh && data.anhBanner === undefined) {
            await dispatch(updateuser({ status: 1, avatar: data.anh, name: data.name, address: data.address, male, phone: data.phone, introduce: content, id: id }));
        } else if (data.anhBanner && data.anh === undefined) {
            await dispatch(updateuser({ status: 1, banner: data.anhBanner, name: data.name, address: data.address, male, phone: data.phone, introduce: content, id: id }));
        } else if (data.anhBanner && data.anh) {
            await dispatch(updateuser({ status: 1, avatar: data.anh, banner: data.anhBanner, name: data.name, male, address: data.address, phone: data.phone, introduce: content, id: id }));
        } else {
            await dispatch(updateuser({ status: 1, name: data.name, address: data.address, phone: data.phone, male, introduce: content, id: id }));
        }
    }
                ...
                <form action="" onSubmit={handleSubmit(onSubmit)}>
                    ...
                    <div className="d-flex">
                        <div className="form-group w-45">
                            ...
                            {loadingTag ? <SpinLoad /> :
                                <Select mode="tags" value={tagId ? tagId : ""} onChange={onChangeTag} className="form-control" placeholder="Tags Mode">
                                    {data}
                                </Select>}
                        </div>
                        <div className="form-group w-45">
